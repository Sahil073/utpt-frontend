      ...
      <div class="resource-card">
        <div class="flex items-center gap-12">
          ...
          <div style="flex:1;overflow:hidden">
            ...
            <div class="t-muted t-xs mt-4" style="margin-top:4px">${r.subject || '—'} · Batch ${r.batch || '—'}</div>
          </div>
        </div>
        ${r.description ? `<div class="t-muted t-sm">${r.description}</div>` : ''}
        <div class="flex items-center justify-between mt-4">
          <div class="flex gap-2">
            <span class="badge badge-default">${r.batch || '—'}</span>
            <span class="badge badge-blue">${r.subject || '—'}</span>
          </div>
...
function applyFilters() {
  loadResources({
    ...
    subject: document.getElementById('r-subject').value.trim(),
  });
}
...
document.getElementById('r-clear-btn').addEventListener('click', () => {
  document.getElementById('r-batch').value   = '';
