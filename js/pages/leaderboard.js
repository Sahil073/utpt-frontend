...
function renderFilterExtra() {
  ...
  if (scope === 'batch') {
    ...
    document.getElementById('lb-go-btn').addEventListener('click', () => {
      batchFilter = document.getElementById('lb-batch-inp').value.trim();
      page = 1; loadLeaderboard();
    });
  } else if (scope === 'spec') {
    el.innerHTML = `<input type="text" class="form-input" id="lb-spec-inp" placeholder="Enter specialization (e.g. CSE)" style="width:220px">
      <button class="btn btn-outline btn-sm" id="lb-go-btn">Go</button>`;
    document.getElementById('lb-go-btn').addEventListener('click', () => {
      specFilter = document.getElementById('lb-spec-inp').value.trim();
      page = 1; loadLeaderboard();
    });
  }
}
...
function renderPodium(rows) {
  ...
  const order = [rows[1], rows[0], rows[2]]; // 2nd, 1st, 3rd
...
function renderTable(rows) {
  ...
  if (!rows?.length) {
    tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><div class="empty-icon">📊</div><div class="empty-title">No data</div><div class="empty-desc">Try a different filter</div></div></td></tr>`;
    return;
  }
  const myId = user?.id;
      ...
      <tr class="${isMe ? 'lb-row-me' : ''}">
        ...
        <td>
          <div class="flex items-center gap-3">
            ${avatarHTML(s, 'sm')}
            <div>
              <div style="font-weight:${isMe ? '700' : '400'}">${s.name || '—'} ${isMe ? '<span class="badge badge-blue" style="font-size:.65rem">You</span>' : ''}</div>
...
async function loadLeaderboard() {
  ...
  try {
    ...
    document.getElementById('lb-tbody').innerHTML =
      `<tr><td colspan="8"><div class="empty-state"><div class="empty-desc">Failed to load</div></div></td></tr>`;
  }
}
