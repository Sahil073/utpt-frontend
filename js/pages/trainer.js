...
if (!requireAuth(['trainer','admin'])) throw new Error('unauthorized');
const user = getUser();

(function() {
  const name=user?.name||'Trainer';
  document.getElementById('sidebar-name').textContent=name;
  document.getElementById('sidebar-role').textContent=user?.role||'trainer';
  ...
  try {
    ...
    tbody.innerHTML=rows.map(s=>`
      <tr>
        ...
        <td><button class="btn btn-outline btn-sm" onclick="viewStudentDetail('${s.id||s.user_id}')">View</button></td>
      </tr>`).join('');
