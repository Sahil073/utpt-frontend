...
let filters = {};
let editingId = null;
let allCount = { easy: 0, medium: 0, hard: 0, total: 0 };
...
if (canManage) {
  document.getElementById('add-q-btn').style.display = 'inline-flex';
  document.getElementById('q-action-th').style.display = '';
}
  ...
  try {
    ...
    tbody.innerHTML = rows.map((q, i) => `
      <tr>
        ...
        <td>
          ${q.platform_link
            ? `<a href="${q.platform_link}" target="_blank" style="font-weight:500;color:var(--text)">${q.title}</a>`
            : `<span style="font-weight:500">${q.title}</span>`}
        </td>
        <td>${diffBadge(q.difficulty)}</td>
        <td>
          <div class="flex gap-2 flex-wrap">
            ${(q.tags || []).map(t => `<span class="badge badge-default">${t}</span>`).join('') || '<span class="t-muted t-sm">—</span>'}
          </div>
        </td>
        <td>
          ${q.platform_link
            ? `<a href="${q.platform_link}" target="_blank" class="platform-link" style="display:inline-flex">
                ${getPlatformIcon(q.platform_link)} →
               </a>`
        ...
        <td>
          <div class="flex gap-2">
            <button class="btn btn-outline btn-sm" onclick="editQuestion('${q.id || q._id}', ${JSON.stringify(q).replace(/"/g, '&quot;')})">Edit</button>
            <button class="btn btn-danger btn-sm" onclick="deleteQuestion('${q.id || q._id}')">Delete</button>
          </div>
        </td>` : ''}
      </tr>`).join('');
    ...
    toast(ex.message || 'Failed to load questions', 'error');
    tbody.innerHTML = `<tr><td colspan="6"><div class="empty-state"><div class="empty-desc">Failed to load</div></div></td></tr>`;
  }
}
...
const tagInput    = document.getElementById('q-tag');
...
window.editQuestion = (id, q) => {
  ...
  document.getElementById('q-link').value       = q.platform_link || '';
  openModal('q-modal');
};
...
document.getElementById('q-form').addEventListener('submit', async e => {
  ...
  const body = {
    ...
    platform_link: document.getElementById('q-link').value.trim() || undefined,
  };
  try {
    if (editingId) await questionsApi.update(editingId, body);
    else           await questionsApi.create(body);
    toast(editingId ? 'Question updated!' : 'Question added!', 'success');
    closeModal('q-modal');
    loadQuestions();
  } catch (ex) {
    toast(ex.message || 'Failed to save', 'error');
    setLoading(btn, false, editingId ? 'Save Changes' : 'Add Question');
  }
});
