...
export function initTabs(containerSel) {
  document.querySelectorAll(containerSel || '.tabs').forEach(tabsEl => {
    tabsEl.querySelectorAll('.tab-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        const panel = btn.dataset.tab;
        tabsEl.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
        const container = tabsEl.closest('.tab-container') || tabsEl.parentElement;
...
export function avatarHTML(user, size = 'md') {
  ...
  if (user?.avatar_url) {
    return `<div class="avatar avatar-${size}" style="background:${col}"><img src="${user.avatar_url}" alt="${name}" loading="lazy"></div>`;
  }
...
// ─── Pagination Helper ─────────────────────────────────────────
export function paginationHTML(page, hasMore, onPage) {
  return `
    <div class="flex gap-2 items-center justify-center mt-6">
      <button class="btn btn-outline btn-sm" ${page <= 1 ? 'disabled' : ''} onclick="(${onPage})(${page - 1})">← Prev</button>
      <span class="t-muted t-sm">Page ${page}</span>
      <button class="btn btn-outline btn-sm" ${!hasMore ? 'disabled' : ''} onclick="(${onPage})(${page + 1})">Next →</button>
    </div>`;
}

// ─── Mini Bar Chart (SVG) ──────────────────────────────────────
export function renderBarChart(container, data, labelKey = 'date', valueKey = 'count', color = '#2f81f7') {
  ...
  const bars = slice.map((d, i) => {
    ...
    const y = H - 20 - h;
    return `<rect class="bar" x="${x.toFixed(1)}" y="${y}" width="${barW}" height="${h}" rx="2" title="${d[labelKey]}: ${d[valueKey]}"/>`;
  }).join('');

  container.innerHTML = `
    <svg class="bar-chart" viewBox="0 0 ${W} ${H}" style="width:100%;height:${H}px">
      ${bars}
