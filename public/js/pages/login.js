...
document
  .querySelectorAll('.tab-btn[data-mode]')
  .forEach(btn => {
...
pwForm.addEventListener('submit', async e => {
  ...
  const id =
    document.getElementById('pw-college-id')
      .value
      .trim();
  ...
  try {
    ...
    console.log(
      'LOCAL USER:',
      localStorage.getItem('utpt_user')
    );

    console.log(
      'LOCAL TOKEN:',
      localStorage.getItem('utpt_token')
    );
...
otpStartForm.addEventListener('submit', async e => {
  ...
  const btn =
    document.getElementById('otp-start-btn');

  const err =
    document.getElementById('otp-start-error');

  err.style.display = 'none';

  collegeId =
    document.getElementById('otp-college-id')
      .value
      .trim();
  ...
  try {
    ...
    document.getElementById(
      ...
      `📧 OTP sent to <strong>${
        data?.email ||
        'your registered @gla.ac.in email'
      }</strong>`;
...
document
  ...
  .addEventListener('submit', async e => {
    ...
    const btn =
      document.getElementById('otp-verify-btn');
